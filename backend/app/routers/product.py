...
def create(product: ProductCreate, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    db_product = create_product(db, product, shop_id=current_user.shop.id)
    return db_product
...
def read_product_by_shop(shop_id: int, db: Session = Depends(get_db)):
    shop_product = get_products_by_shop(db, shop_id)
    return shop_product
